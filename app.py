...
with st.sidebar:
    ...
    impuestos = st.number_input("Impuestos (Contador)", value=4575, step=100)
    contador = st.number_input("Honorarios Contador", value=30000, step=1000)
    administracion = st.number_input("Administración", value=100000, step=1000)
...
if archivo is not None:
    ...
    kines_especial = ['José Miguel Gomez Albornoz']
    ...
    for kine in df['Prestador'].unique():
        ...
        if kine in kines_full_time:
            ...
            bono = (cant_epi * 13500) + (cant_eco * 3500)
            total_imposiciones += imposicion_pt
        elif kine in kines_especial:
            base = 0
            bono = kine_df_efectivo['Precio real'].sum() * 0.60
    ...
    ingreso_epi_bruto = cant_epi_global * 45000
    ingreso_eco_bruto = cant_eco_global * 25000
    
    eco_data = [
        {
            "Servicio": "EPI", 
            "Cobro a Cliente": "$45.000", 
            "Ingreso Bruto Total": f"${ingreso_epi_bruto:,.0f}", 
            "Costo Máquina": "$18.000", 
            "Cantidad": cant_epi_global, 
            "Costo Total Máquina": f"${cant_epi_global * 18000:,.0f}",
            "Pago a Profesional": "$13.500",
            "Pago Total Profesionales": f"${cant_epi_global * 13500:,.0f}"
        },
        {
            "Servicio": "Ev. Ecográfica", 
            "Cobro a Cliente": "$25.000", 
            "Ingreso Bruto Total": f"${ingreso_eco_bruto:,.0f}", 
            "Costo Máquina": "$10.000", 
            "Cantidad": cant_eco_global, 
            "Costo Total Máquina": f"${cant_eco_global * 10000:,.0f}",
            "Pago a Profesional": "$3.500",
            "Pago Total Profesionales": f"${cant_eco_global * 3500:,.0f}"
        }
    ]
    ...
    ingreso_potencial = ingreso_total + dinero_no_pagado
    
    st.markdown("**Resumen de Flujo Potencial**")
    resumen_deuda_data = [
        {"Concepto": "Ingreso Real (Caja Actual)", "Monto": f"${ingreso_total:,.0f}"},
        {"Concepto": "Dinero No Pagado (Deuda)", "Monto": f"${dinero_no_pagado:,.0f}"},
        {"Concepto": "Ingreso Potencial (Caja + Deuda)", "Monto": f"${ingreso_potencial:,.0f}"}
    ]
    st.dataframe(pd.DataFrame(resumen_deuda_data), use_container_width=True)
    
    st.markdown("**Detalle de Pacientes Deudores**")
    ...
    estados_validos = [col for col in ['Asiste', 'Confirmado', 'Reservado'] if col in tabla_estados.columns]
    tabla_estados['Total Efectivo'] = tabla_estados[estados_validos].sum(axis=1)
    ...
    # ------------------ SECCIÓN 8: CUADRO RESUMEN FINAL ------------------
    ...
    datos_resumen = [
        ...
        ["Dinero No Pagado (Deuda)", dinero_no_pagado],
        ["Ingreso Potencial (Caja + Deuda)", ingreso_potencial],
        ...
        ["Sueldo José Miguel Gomez Albornoz", dict_sueldos.get('José Miguel Gomez Albornoz', 0)],
